...
export function localPlan(text) {
  ...
  if (/^(send|text|sms)\s+/i.test(input)) {
    const {contact, message} = smsFromText(input);
    return plan('SEND_MESSAGE', 'sms.compose', {platform: 'SMS', recipient: contact, message}, true, 'I can prepare the SMS for your review before sending.');
  }
